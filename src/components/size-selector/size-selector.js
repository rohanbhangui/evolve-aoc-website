    ...
    return (
      <div className="size-selector">
        ...
        <div className={`label-group ${ sizeRequiredError ? 'error-active' : ''}`}>
          ...
          <label className={`${inventoryCounts.xxl === 0 ? 'disabled' : '' }`}><input name={ `size-selector-${id}`} checked={ selectedSize==="XXL"} disabled={inventoryCounts.xxl === 0} type="radio" onChange={ selectedSizeHandler('XXL') } defaultValue="XXL"/><span>XXL</span></label>
        </div>
      </div>
    )
  }
